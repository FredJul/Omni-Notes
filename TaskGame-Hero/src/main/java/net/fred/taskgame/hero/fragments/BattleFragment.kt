...
class BattleFragment : BaseFragment() {
    ...
    override val mainMusicResId: Int
        @RawRes
        get() {
            if (level != null && level!!.battleMusicResId != Level.INVALID_ID) {
                return level!!.battleMusicResId
            }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if (savedInstanceState != null) {
            ...
            battleManager.addEnemyCards(level!!.enemyCards)
    ...
    override fun onViewCreated(view: View?, savedInstanceState: Bundle?) {
        ...
        enemy_portrait.setImageResource(level!!.getEnemyIcon(context))
    ...
    fun onUseCardClicked() {
        ...
        val playedCard = currentlyAnimatedCard!!.card!!
        if (playedCard.type == Card.Type.CREATURE) {
            val cardPaddingStart = currentlyAnimatedCard!!.paddingStart - select_card_bottom_sheet.scrollX
            val cardPaddingTop = currentlyAnimatedCard!!.paddingTop + select_card_bottom_sheet.paddingTop
            currentlyAnimatedCard!!.animate().scaleX(1f).scaleY(1f).translationX(0f).translationY(0f)
                    ...
                    .withEndAction {
                        ...
                        player_card.card = currentlyAnimatedCard!!.card
            ...
            currentlyAnimatedCard!!.animate().alpha(0f)
                    .withEndAction {
                        mainActivity?.playSound(MainActivity.SOUND_USE_SUPPORT)
                        currentlyAnimatedCard!!.animate().alpha(0f).withEndAction {
                            card_list.removeView(currentlyAnimatedCard)
                            currentlyAnimatedCard = null
                            battleManager.play(playedCard)
                            animateNextStep()
    ...
    fun onAleatoryStrategyButtonClicked() {
        val result = battleManager.applyNewStrategy(BattleManager.BattleStrategy.ALEATORY)
    ...
    private fun animateNextStep() {
        ...
        when (step) {
            ...
            BattleManager.BattleStep.FIGHT -> {
                ...
                if (player_card.card!!.useMagic) {
                    mainActivity?.playSound(MainActivity.SOUND_FIGHT_MAGIC)
                } else if (player_card.card!!.useWeapon) {
                ...
                enemy_card.postDelayed({
                    if (mainActivity != null) {
                        if (enemy_card.card!!.useMagic) {
                            mainActivity?.playSound(MainActivity.SOUND_FIGHT_MAGIC)
                        } else if (enemy_card.card!!.useWeapon) {
                            mainActivity?.playSound(MainActivity.SOUND_FIGHT_WEAPON)
                        } else {
                            mainActivity?.playSound(MainActivity.SOUND_FIGHT)
                        }
            ...
            BattleManager.BattleStep.PLAYER_WON -> {
                if (battleManager.lastUsedPlayerCreatureCard!!.defense <= 0) {
            ...
            BattleManager.BattleStep.ENEMY_WON -> {
                ...
                val dialog = EndBattleDialogFragment.newInstance(level!!, level!!.isCompleted, EndBattleDialogFragment.EndType.ENEMY_WON)
            ...
            BattleManager.BattleStep.DRAW -> {
                ...
                val dialog = EndBattleDialogFragment.newInstance(level!!, level!!.isCompleted, EndBattleDialogFragment.EndType.DRAW)
                val transaction = fragmentManager.beginTransaction().addToBackStack(null)
                dialog.show(transaction, EndBattleDialogFragment::class.java.name)
            }
        }
    ...
    private fun displayVictory() {
        if (mainActivity != null && isVisible) {
            ...
            val dialog = EndBattleDialogFragment.newInstance(level!!, level!!.isCompleted, EndBattleDialogFragment.EndType.PLAYER_WON)
            val transaction = fragmentManager.beginTransaction().addToBackStack(null)
            dialog.show(transaction, EndBattleDialogFragment::class.java.name)
        }
    ...
    fun onDarkLayerClicked() {
        ...
        currentlyAnimatedCard!!.animate().scaleX(1f).scaleY(1f).translationX(0f).translationY(0f).withEndAction { updateBottomSheetUI() }
        currentlyAnimatedCard = null
    }
    ...
    private fun highlightCard(cardView: GameCardView) {
        ...
        selectCardBottomSheetBehavior!!.isHideable = true
        selectCardBottomSheetBehavior!!.state = BottomSheetBehavior.STATE_HIDDEN

        currentlyAnimatedCard = cardView
        cardView.animate()
                .scaleX(1.7f)
                .scaleY(1.7f)
                .translationX(view!!.width / 2f - cardView.width / 2f - cardView.x + select_card_bottom_sheet.scrollX)
    ...
    private fun onStrategySelected() {
        selectStrategyBottomSheetBehavior!!.isHideable = true
        selectStrategyBottomSheetBehavior!!.state = BottomSheetBehavior.STATE_HIDDEN
    ...
    private fun updateBottomSheetUI() {
        if (battleManager.currentStep == BattleManager.BattleStep.SELECT_STRATEGY) {
            if (selectStrategyBottomSheetBehavior!!.isHideable) {
                selectStrategyBottomSheetBehavior!!.isHideable = false
                selectStrategyBottomSheetBehavior!!.state = BottomSheetBehavior.STATE_COLLAPSED
            }

            if (!selectCardBottomSheetBehavior!!.isHideable) {
                selectCardBottomSheetBehavior!!.isHideable = true
                selectCardBottomSheetBehavior!!.state = BottomSheetBehavior.STATE_HIDDEN
            }
        } else {
            ...
            if (card_list.childCount > 0) {
                if (selectCardBottomSheetBehavior!!.isHideable) {
                    selectCardBottomSheetBehavior!!.isHideable = false
                    selectCardBottomSheetBehavior!!.state = BottomSheetBehavior.STATE_COLLAPSED
                }
            } else {
                if (!selectCardBottomSheetBehavior!!.isHideable) {
                    selectCardBottomSheetBehavior!!.isHideable = true
