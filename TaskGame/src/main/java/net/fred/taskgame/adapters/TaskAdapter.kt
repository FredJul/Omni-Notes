...
class TaskAdapter(private val mItemActionListener: ItemActionListener, recyclerView: RecyclerView, private var _tasks: MutableList<Task>) : MultiSelectAdapter<TaskAdapter.TaskViewHolder>(mItemActionListener, recyclerView) {
    ...
    class TaskViewHolder(v: View) : RecyclerView.ViewHolder(v), ItemActionViewHolder {
        ...
        override fun onItemSelected() {
            ...
            itemView.root.setBackgroundResource(R.color.gray_bg)
            itemView.root.elevation = 1f
        }

        override fun onItemClear() {
            itemView.root.setBackgroundColor(Color.WHITE)
            itemView.root.elevation = 0f
        }
    }
            ...
            if (holder.adapterPosition != -1) { // seems -1 is sometimes possible...
                if (selectedItemCount > 0) {
                    if (isItemSelected(holder.adapterPosition)) {
                        ...
                        mItemActionListener.onItemUnselected(holder.adapterPosition)
                    } else {
                        toggleSelection(true, holder.adapterPosition)
                        mItemActionListener.onItemSelected(holder.adapterPosition)
                    }
                } else {
                    mItemActionListener.onItemClicked(holder.adapterPosition)
                }
            }
        }
